...
function loadImages(brand, model, id){
  ...
  document.getElementById("FilterMake").innerHTML = "";

function FilterMake(){
    // Get the dropdown element
    var makeDropdown = document.getElementById("makeDropdown");
    var selectOption = makeDropdown.value ;
    ...
    xhttp.onload = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var inner = document.getElementById("FilterMake");
...
var opt = JSON.stringify({
    ...
    xhttp.open("POST", "https://wheatley.cs.up.ac.za/u22561154/api.php", true);
    xhttp.send(opt);
